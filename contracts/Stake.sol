...
contract Stake {
    ...
    struct Senders {
        bool allowed;
        uint256 amount_sends;
        mapping(uint256 => WithdrawlStruct) withdrawls;
    }
    ...
    function sendFunds(uint256 amount, address receiver) returns (uint256) {
        if (isAllowedToSend(msg.sender)) {
            if (this.balance >= amount) {
                ...
                isAllowedToSendFundsMapping[msg.sender]
                    .withdrawls[isAllowedToSendFundsMapping[msg.sender]
                    .amount_sends]
                    .to = receiver;
                isAllowedToSendFundsMapping[msg.sender]
                    .withdrawls[isAllowedToSendFundsMapping[msg.sender]
    ...
    function isAllowedToSend(address _address) constant returns (bool) {
        return
            isAllowedToSendFundsMapping[_address].allowed || _address == owner;
    }
